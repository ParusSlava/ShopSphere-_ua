...
### SQL-запит для аналізу регіональної динаміки

Для аналізу розвитку регіонів було використано таблиці `orders` та `customers`.

Таблицю `orders` об'єднано з `customers` за `customer_id`, щоб для кожного замовлення визначити регіон клієнта.

Після цього дані агреговано за:
- регіоном (`region`);
- роком (`order_year`).

Для кожної комбінації регіон × рік розраховано:
- `orders_count` — кількість унікальних замовлень;
- `total_net_revenue` — сумарну чисту виручку.

Отриманий набір даних використано в Tableau для побудови графіка регіональної динаміки за 2022–2024 роки.
